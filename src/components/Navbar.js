...
const Navbar = () => {
  ...
  const handleLogout = () => {
    ...
    localStorage.removeItem("token");    // token remove (agar hai)
    navigate("/", { replace: true });    // redirect to homepage
    // OR window.location.href = "http://localhost:3000/";
  };
    ...
    <nav className="fixed top-0 left-0 right-0 bg-primary py-4 font-poppins z-50 shadow-md">
      <div className="container mx-auto px-6 flex justify-between items-center">
        ...
        <div className="flex items-center space-x-4">
          ...
          <Link to="/browse" className="text-text-light hover:text-accent px-3 py-2 rounded-md text-xs font-medium">
            Browse Events
          </Link>
